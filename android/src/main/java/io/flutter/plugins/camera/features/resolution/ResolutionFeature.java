...
public class ResolutionFeature extends CameraFeature<ResolutionPreset> {
 ...
 rn cameraCharacteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
    // if (preset.ordinal() > ResolutionPreset.high.ordinal()) {
    //   preset = ResolutionPreset.high;
    // }
   ...
   *                  
  /**   
   *                   the best possible {@link android.media.CamcorderProfile} for the supplied {@link
   * ResolutionPreset}.
   *          
   * @param cameraId Camera identifier which indicates the device's camera for which to select a
   *     {@link android.media.CamcorderProfile}.param preset 
      he {@link ResolutionPreset} for which is to be translated to a {@link
    ...
    switch (preset) {
      ...
      case low:
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_QVGA)) {
          return CamcorderProfile.get(cameraI
      default:
